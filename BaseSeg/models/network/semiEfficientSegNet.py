...
__all__ = ['SemiEfficientSegNet']


@NETWORK_REGISTRY.register()
class SemiEfficientSegNet(nn.Module):
    def __init__(self, cfg=None):
        super().__init__()

        # semiEfficientSegNet parameter.
        ...
        self.semi_supervised = cfg.semi_supervised
        self.is_dynamic_empty_cache = cfg.is_dynamic_empty_cache if not self.semi_supervised else False

        encoder_conv_block = BLOCK_REGISTRY.get(cfg.encoder_conv_block)
        decoder_conv_block = BLOCK_REGISTRY.get(cfg.decoder_conv_block)
        # if cfg.context_block is not None:
        ...
        if self.semi_supervised:
            self.final4 = nn.Sequential(
                ...
                ConvINReLU3D(num_channel[4], num_channel[4], kernel_size=3, padding=1, p=0.2),
                nn.Conv3d(num_channel[4], num_class, kernel_size=1, bias=False)
            )
            self.final3 = nn.Sequential(
                nn.Upsample(scale_factor=8, mode='trilinear', align_corners=True),
                ConvINReLU3D(num_channel[3], num_channel[3], kernel_size=3, padding=1, p=0.2),
                nn.Conv3d(num_channel[3], num_class, kernel_size=1, bias=False)
            )
            self.final2 = nn.Sequential(
                nn.Upsample(scale_factor=4, mode='trilinear', align_corners=True),
                ConvINReLU3D(num_channel[2], num_channel[2], kernel_size=3, padding=1, p=0.2),
                nn.Conv3d(num_channel[2], num_class, kernel_size=1, bias=False)
            )
            self.final1 = nn.Sequential(
                nn.Upsample(scale_factor=2, mode='trilinear', align_corners=True),
                ConvINReLU3D(num_channel[1], num_channel[1], kernel_size=3, padding=1, p=0.2),
                nn.Conv3d(num_channel[1], num_class, kernel_size=1, bias=False)
            )
        self.final0 = nn.Conv3d(num_channel[0], num_class, kernel_size=1, bias=False)
    ...
    def forward(self, x):
        ...
        if self.semi_supervised:
            x_4 = self.final4(x4_0)
            x_3 = self.final3(x3_0)
            x_2 = self.final2(x2_0)
            x_1 = self.final1(x1_0)
            # return [x_4, x_3, x_2, x_1, x]
            return [x_2, x_1, x]
        else:
            return x
